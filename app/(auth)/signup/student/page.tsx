...
import { ArrowRight, ArrowLeft, CheckCircle, Eye, EyeOff, GraduationCap } from 'lucide-react'
...
const STEPS = ['Account', 'Education', 'Goals', 'Done']

const gradYears = ['2025', '2026', '2027', '2028'].map(y => ({ value: y, label: y }))
const hearAbout = [
  { value: 'social', label: 'Social Media' }, { value: 'friend', label: 'Friend / Referral' },
...
export default function StudentSignupPage() {
  ...
  const [form, setForm] = useState({
    ...
    college: '', field: '', gradYear: '',
    wantToLearn: '', heardFrom: '', agreement: false,
  })
  ...
  const validate = (s: number) => {
    ...
    if (s === 2) {
      ...
      if (!form.gradYear) e.gradYear = 'Required'
    }
    if (s === 3) {
      if (!form.wantToLearn.trim()) e.wantToLearn = 'Required'
  ...
  const handleFinish = async () => {
    ...
    setIsLoading(true)
    setSignupError('')
    const result = await signup({ email: form.email, password: form.password, name: form.name, role: 'student' })
    if (result.success) {
      setStep(4)
    } else {
    ...
    <motion.div className="w-full" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: EASE }}>
      <AuthCard className="max-w-[540px]">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="text-[12px] font-bold uppercase tracking-[0.1em] text-dark/30">Student Signup</span>
        </div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.35, ease: EASE }} className="space-y-5">
              <Input label="Full name" required placeholder="Priya Sharma" value={form.name} onChange={e => set('name', e.target.value)} error={errors.name} />
              ...
              <Input label="Phone" type="tel" required placeholder="+91 98765 43210" value={form.phone} onChange={e => set('phone', e.target.value)} error={errors.phone} />
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.35, ease: EASE }} className="space-y-5">
              <Input label="College / University" required placeholder="e.g., Delhi University" value={form.college} onChange={e => set('college', e.target.value)} error={errors.college} />
              <Input label="Field of study" required placeholder="e.g., Computer Science" value={form.field} onChange={e => set('field', e.target.value)} error={errors.field} />
              <Select label="Graduation year" required options={gradYears} placeholder="Select" value={form.gradYear} onChange={e => set('gradYear', e.target.value)} error={errors.gradYear} />
          ...
          {step === 3 && (
            <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.35, ease: EASE }} className="space-y-5">
              <Textarea label="What do you want to learn?" required placeholder="Skills, tools, or areas you're interested in..." value={form.wantToLearn} onChange={e => set('wantToLearn', e.target.value)} error={errors.wantToLearn} />
              <Select label="How did you hear about us?" options={hearAbout} placeholder="Select" value={form.heardFrom} onChange={e => set('heardFrom', e.target.value)} />
              <label className="flex items-start gap-3 cursor-pointer group">
                ...
                <span className="text-[13px] text-dark/40 group-hover:text-dark/55 transition-colors leading-relaxed">I commit to attending scheduled sessions and making the most of this mentorship opportunity.</span>
              </label>
          ...
          {step === 4 && (
            <motion.div key="s4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: EASE }} className="text-center py-8">
              ...
              <h2 className="text-[1.5rem] font-semibold text-dark tracking-[-0.02em] mb-2">Welcome aboard!</h2>
              <p className="text-[14px] text-dark/40 mb-8 max-w-sm mx-auto">We&apos;ll match you with a mentor and reach out with your first session details soon.</p>
              <Button variant="dark" size="lg" href="/dashboard/student" icon={<ArrowRight className="w-4 h-4" />}>Go to Dashboard</Button>
            </motion.div>
          )}
        </AnimatePresence>
